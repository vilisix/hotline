...
FuzzyScore FuzzyScorer::GetFuzzyScore(const std::string &query, const std::string &queryLower, int querySize, const std::string &target, const std::string &targetLower, int targetSize)
{
    ...
    for(int queryIndex = 0; queryIndex < querySize; queryIndex++){
        ...
        for(int targetIndex = 0; targetIndex < targetSize; targetIndex++){
            ...
            if (!diagScore && queryIndexGtNull) {
                ...
                score = ComputeCharScore(queryCharAtIndex, queryLowerCharAtIndex, targetChar, targetLowerChar, targetIndex, matchesSequenceLength);
            }
    ...
    return {target, scores[querySize * targetSize - 1], positions};
}

int FuzzyScorer::ComputeCharScore(const char &queryChar, const char &queryCharLower, const char &targetChar, const char &targetCharLower, int targetIdx, int sequenceMatch)
{
    int score = 0;
